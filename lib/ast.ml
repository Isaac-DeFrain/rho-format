...
let rec view_proc ind ?(cfg = std_cfg) = function
...
| New ((h, tl), p) -> String.make ind ' ' ^ "new " ^ view_names ind cfg (Names (h :: tl)) ^ "{" ^ view_proc (ind + cfg.indent) ~cfg p ^ "}"
...
| Receive (bs, p) -> wrap_with_cfg ind cfg (For (view_binds ind cfg bs)) @@ view_proc (ind + cfg.indent) ~cfg p
...
| Wildcard -> String.make ind ' ' ^ "_"
| PatVar x -> String.make ind ' ' ^ x
...
and wrap_with_cfg _ind cfg =
function
| Braces ->
    fun s ->
      if cfg.new_line_brace then sprintf "{\n %s \n}" s
      else if cfg.fluffy_brace then sprintf "{ %s }" s
      else sprintf "{%s}" s
| Parens ->
    fun s ->
      if cfg.fluffy_paren then sprintf "( %s )" s
      else sprintf "(%s)" s
| For bs ->
    fun s ->
      sprintf (
        if cfg.fluffy_for then "for (%s) {%s}"
        else "for(%s){%s}")
        bs
        s
| Send (per, ch) ->
    fun s ->
      sprintf (
        if cfg.fluffy_paren then "%s%s( %s )"
        else "%s%s(%s)")
        ch
        (if per then "!!" else "!")
        s
